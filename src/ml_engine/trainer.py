...
try:
    from gymnasium import Env as _GymEnv  # type: ignore[import]
except ImportError as _gym_import_err:
    raise ImportError(
        "[!] gymnasium is required for RL training. "
        "Add 'gymnasium>=0.29.0' to your Dockerfile."
    ) from _gym_import_err
...
class DataShaper:
    ...
    def split(
        ...
        then slices into three contiguous temporal windows. No row is
...
class BaseTrainer(ABC):
    ...
    @abstractmethod
    def train(
        self, split: TrainValTestSplit, is_incremental: bool = False
    ) -> TrainerResult:
        """
...
class XGBoostTrainer(BaseTrainer):
    ...
    def train(
        ...
        try:
            ...
            # Incremental retrains use a very small eta (0.01) to nudge the
            # model toward recent data without overwriting "Master Model"
            # weights learned during the 5-year boot.  Full retrains use the
            # XGBoost default eta of 0.3.
            current_lr: float = 0.01 if is_incremental else 0.3
...
class LightGBMTrainer(BaseTrainer):
    ...
    def train(
        ...
        Args:
            split:          Time-ordered TrainValTestSplit from DataShaper.
            is_incremental: When True, divides the learning rate by 10 to
                            protect "Master Model" weights during hourly
                            retrains on small datasets.
        ...
        try:
            ...
            self._feature_names = list(split.feature_names)

            x_train_df = pd.DataFrame(split.x_train, columns=self._feature_names)
            x_val_df = pd.DataFrame(split.x_val, columns=self._feature_names)
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        self._check_trained()
        x_input: Any = (
            pd.DataFrame(X, columns=self._feature_names) if self._feature_names else X
        )
...
class RandomForestTrainer(BaseTrainer):
    ...
    def train(
        ...
        Args:
            split:          Time-ordered TrainValTestSplit from DataShaper.
            is_incremental: Accepted for API compatibility with BaseTrainer.
                            RandomForest does not use a learning rate, so
                            this flag has no effect on model construction.
...
class StackingEnsembleTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        if not base_predictions:
            raise ValueError(
                "[!] StackingEnsembleTrainer requires at least one base model prediction. base_predictions is empty."
            )
    ...
    def train(
        ...
        Args:
            split:          TrainValTestSplit — used for y_val labels and
                            metadata. X_train and X_val are not used directly;
                            the meta-features come from base_predictions.
            is_incremental: Accepted for API compatibility with BaseTrainer.
                            The meta-learner is always retrained from scratch
                            on the stacked base predictions, so this flag has
                            no effect on model construction.
...
class LSTMTrainer(BaseTrainer):
    ...
    def train(
        ...
        Args:
            ...
            is_incremental: When True, divides the learning rate by 10 to
                            protect "Master Model" weights during hourly
                            retrains on small datasets (1,440 bars).
...
class _ForexBinaryEnv(_GymEnv):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "[!] gymnasium is required for RL training. Add 'gymnasium>=0.29.0' to your Dockerfile."
            ) from exc
...
class _RLBaseTrainer(BaseTrainer):
    ...
    def train(
        ...
        Args:
            split:          TrainValTestSplit — X_train/y_train used for
                            the training environment; X_val/y_val for the
                            eval episode.
            is_incremental: When True, subclasses should scale their
                            learning rate down before calling this method
                            (see RecurrentPPOTrainer for the pattern).
                            A2C, DQN, and PPO accept the flag for API
                            compatibility but do not alter their lr here;
                            override train() in those subclasses if needed.
...
class A2CTrainer(_RLBaseTrainer):
    ...
    def __init__(
        ...
        super().__init__(expiry_key, features, labels, n_steps_train, payout_ratio)
        self.learning_rate = learning_rate
    ...
    def _build_sb3_model(self, env: _ForexBinaryEnv) -> Any:
        try:
            ...
            raise ImportError(
                "[!] stable-baselines3 is required for A2CTrainer. Add 'stable-baselines3>=2.3.0' to your Dockerfile."
            ) from exc
...
class DQNTrainer(_RLBaseTrainer):
    ...
    def _build_sb3_model(self, env: _ForexBinaryEnv) -> Any:
        try:
            ...
            raise ImportError(
                "[!] stable-baselines3 is required for DQNTrainer. Add 'stable-baselines3>=2.3.0' to your Dockerfile."
            ) from exc
...
class PPOTrainer(_RLBaseTrainer):
    ...
    def _build_sb3_model(self, env: _ForexBinaryEnv) -> Any:
        try:
            ...
            raise ImportError(
                "[!] stable-baselines3 is required for PPOTrainer. Add 'stable-baselines3>=2.3.0' to your Dockerfile."
            ) from exc
...
class RecurrentPPOTrainer(_RLBaseTrainer):
    ...
    def _build_sb3_model(self, env: _ForexBinaryEnv) -> Any:
        try:
            ...
            raise ImportError(
                "[!] sb3-contrib is required for RecurrentPPOTrainer. Add 'sb3-contrib>=2.3.0' to your Dockerfile."
            ) from exc
    ...
    def train(
        ...
        """
        Train the RecurrentPPO agent, scaling the learning rate for
        incremental (hourly) retrains to protect "Master Model" weights.

        When ``is_incremental=True`` the learning rate is divided by 10
        *before* ``_build_sb3_model()`` constructs the SB3 model, so the
        reduced rate is baked into the policy optimiser from the start.
        The original ``self.learning_rate`` is restored after training so
        the object can be reused for a full retrain without re-instantiation.

        Args:
            split:          Time-ordered TrainValTestSplit from DataShaper.
            is_incremental: When True, divides ``self.learning_rate`` by 10
                            before building the SB3 model.

        Returns:
            TrainerResult: Trained RecurrentPPO artifact with RL metrics.
        ...
        try:
            ...
            # Always restore the original lr so the trainer object remains
            # reusable for a subsequent full retrain without re-instantiation.
