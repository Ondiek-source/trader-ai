...
----------
  ...
  FeatureEngineer.evaluate_gates(fe)                 -> TradeEligibility
...
class TradeEligibility:
    """
    Gate evaluation result for the latest feature row.

    Produced by ``FeatureEngineer.evaluate_gates()``. A trade is eligible
    only if ALL four gates pass simultaneously; a single failure blocks entry.

    Attributes:
        is_eligible:  True only when every gate in ``gate_results`` is True.
        gate_results: Per-gate boolean outcomes keyed by gate expression string.
        gate_values:  Per-gate raw numeric values for diagnostic logging.
    """
    ...
    def to_dict(self) -> dict:
        """Return a JSON-serialisable snapshot of the gate evaluation."""
        return {
    ...
    def __repr__(self) -> str:
        passed = sum(self.gate_results.values())
        total = len(self.gate_results)
        return (
            f"TradeEligibility("
            f"eligible={self.is_eligible}, "
...
class FeatureEngineer:
    ...
    def transform(
        ...
        if bars.empty:
            warning_block = (
                f"\n{'%' * 60}\n"
                f"FEATURE ENGINEERING WARNING: EMPTY BARS DATAFRAME\n"
                f"Input bars DataFrame is empty. Returning empty features.\n"
                f"{'%' * 60}"
            )
        ...
        if not isinstance(bars.index, pd.DatetimeIndex):
            logger.warning(
                "[%%] FEATURE ENGINEERING: bars index is not DatetimeIndex "
                "(type=%s) — coercing to datetime. Verify source data formatting.",
        ...
        if missing:
            error_block = (
                f"\n{'!' * 60}\n"
                f"FEATURE ENGINEERING FAILURE: MISSING COLUMNS\n"
        ...
        # Schema validation: warn if any primary feature columns are absent.
        # This catches misconfigured feature flags (e.g. feat_momentum_enabled=False)
        # that would silently deliver a zero-filled vector to the model — which
        # was trained on a full 50-feature schema and will produce garbage output.
        # This is a WARNING not a raise: disabled groups are intentional during
        # development. The caller (engine) decides whether to proceed or abort.
        all_primary = [
    ...
    def _compute_price_action(self, fe: pd.DataFrame) -> pd.DataFrame:
        ...
        prev_body_abs = (prev_close - prev_open).abs()
    ...
    def _compute_derived(self, fe: pd.DataFrame) -> pd.DataFrame:
        ...
        # NOTE: These rolling averages are computed on the already-computed
        # primary feature series, not on raw bar data. pandas .rolling() is
        # always backward-looking (window = [t-N+1 … t]) so there is no
        # look-ahead bias. The quality of these thresholds is bounded by the
        # quality of the input: any forward-filled gaps that survived the
        # _MAX_FFILL_BARS limit will produce artificially flat gate thresholds
        # for those windows. This is a data-quality dependency, not a bias.
        ...
        # Alias RVOL for gate readability
        if "RELATIVE_VOLUME_RVOL" in fe.columns:
            fe["RVOL"] = fe["RELATIVE_VOLUME_RVOL"]

        return fe

    # ── Eligibility Gate Evaluator ───────────────────────────────────────────

    def evaluate_gates(self, fe: pd.DataFrame) -> TradeEligibility:
        """
        ...
        Returns:
            TradeEligibility with per-gate booleans and values.
        """
        if fe.empty:
            return TradeEligibility(
                is_eligible=False,
                gate_results={g: False for g in TRADE_ELIGIBILITY_GATES},
                gate_values={},
            )
        ...
        gate_results[f"RVOL > {self._settings.gate_min_rvol}"] = (
            rvol_val > self._settings.gate_min_rvol
        )
        gate_values["RVOL"] = rvol_val

        # Gate 4: Tight spread
        spread_val = float(row.get("SPREAD_NORMALIZED", 999))
        gate_results[f"SPREAD_NORMALIZED < {self._settings.gate_max_spread}"] = (
            spread_val < self._settings.gate_max_spread
        )
        gate_values["SPREAD_NORMALIZED"] = spread_val
