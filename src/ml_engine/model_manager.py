...
from core.exceptions import ModelManagerError
...
class ModelManager:
    ...
    def save(self, result: TrainerResult) -> str:
        ...
        base_name: str = f"{result.model_name}_{safe_symbol}_{result.expiry_key}"
    ...
    def load(
        ...
        self._check_version(model_version, path.name)
    ...
    def _check_version(self, model_version: str, artifact_name: str) -> None:
        """Validate feature version; raise if stale and not allowed, warn if allowed."""
    ...
    def _resolve_model(
        ...
        """Dispatch deserialisation to the correct backend (SB3 / PyTorch / joblib)."""
        if is_sb3:
    ...
    async def pull_from_blob(
        ...
        return await asyncio.to_thread(
            self._pull_from_blob_sync, symbol, expiry_key, model_name
        )

    def _pull_from_blob_sync(
        self,
        symbol: str,
        expiry_key: str,
        model_name: str | None,
    ) -> str | None:
        """Synchronous blob listing + download — called via asyncio.to_thread."""
        try:
            blobs = list(
                self._storage._container_client.list_blobs(name_starts_with="models/")  # type: ignore[union-attr]
            )
    ...
    def validate_metadata(self, metadata_path: str) -> dict[str, Any]:
        ...
        # Freeze the loaded dict so downstream code cannot mutate the metadata
        # after validation. All reads go through the immutable proxy; any attempt
        # to modify the mapping raises TypeError at the call site.
        metadata: Mapping[str, Any] = MappingProxyType(raw)

        # Derive missing field names by iterating our own constant set and checking
        # membership in metadata. This ensures the logged field names always originate
        # from our controlled constants — never from the externally-supplied JSON —
        # which breaks the taint chain that would otherwise reach the logger sink.
    ...
    def _safe_unlink(path: Path) -> None:
        """Remove a file silently — used to clean up partial writes on failure."""
        if path.exists():
            try:
                path.unlink()
            except OSError:
    ...
    @staticmethod
    def _sanitize_log(value: object, max_len: int = 256) -> str:
        """Strip control characters from external values before passing to logger.

        Prevents log injection via newlines or ANSI sequences embedded in
        file-sourced metadata (artifact filenames, JSON parse errors, etc.).
        """
        return (
            str(value)[:max_len]
            .replace("\n", "\\n")
    ...
    def _load_sb3(path: Path, metadata: dict[str, Any]) -> Any:
        ...
        if module_path is None:
            ...
            raise ModelManagerError(
                ...
                "to _SB3_CLASS_MAP in ModelManager._load_sb3().",
                stage="load",
            )
