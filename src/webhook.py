...
class WebhookSender:
    ...
    Example::
        ...
        # Async usage (preferred in asyncio context)
        result = await sender.send_async({"side": "buy", "symbol": "EURUSD", "key": "Ondiek"})
    ...
    async def send_async(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        for attempt in range(1, self.MAX_ATTEMPTS + 1):
            ...
            try:
                # Run the blocking HTTP call in a thread executor
                # so we don't freeze the event loop
                resp: (
                    requests.Response
    ...
    def _log_attempt(
        ...
        """
        is_success: bool = status is not None and 200 <= status < 300
        record: dict[str, Any] = {
