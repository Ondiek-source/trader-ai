...
_VALID_SIDES: frozenset[str] = frozenset(
    {
        "CALL",
        "PUT",
    }
    ...
    def __post_init__(self) -> None:
        ...
        if self.timestamp.tzinfo is not None:
            if self.timestamp.utcoffset() != timedelta(0):
                logger.critical(
                    {
                        "event": "TRADE_ENTRY_TIMEZONE_REJECTED",
                        "symbol": self.symbol,
                        "side": self.side,
        ...
        if self.entry_price <= 0:
            violations.append(f"Invalid entry_price: {self.entry_price} (must be > 0)")
        if self.exit_price <= 0:
            violations.append(f"Invalid exit_price: {self.exit_price} (must be > 0)")
        ...
        if violations:
            logger.critical(
                {
                    "event": "TRADE_ENTRY_VALIDATION_FAILED",
                    "symbol": self.symbol,
                    "side": self.side,
    ...
    def __post_init__(self) -> None:
        ...
        if self.timestamp.tzinfo is not None:
            if self.timestamp.utcoffset() != timedelta(0):
                logger.critical(
                    {
                        "event": "SIGNAL_ENTRY_TIMEZONE_REJECTED",
                        "symbol": self.symbol,
        ...
        if violations:
            logger.critical(
                {
                    "event": "SIGNAL_ENTRY_VALIDATION_FAILED",
                    "symbol": self.symbol,
                    "direction": self.direction,
                    "violations": violations,
                }
...
class Journal:
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            logger.critical(
                {
                    "event": "JOURNAL_INIT_FAILED",
                    "directory": str(self.journal_dir),
                    "error": str(e),
