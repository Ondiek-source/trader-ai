...
<script>
  ...
  async function refresh() {
    try {
      ...
      const evt = d.last_event || '';
      if (evt && evt !== lastEventHash) {
        lastEventHash = evt;
        activity.unshift({ time: new Date().toLocaleTimeString(), text: evt, type: evtType(evt) });
            ...
            trades.append(
                {
                    ...
                    "result": (
                        "win"
                        if result_val > 0
                        else "loss" if result_val < 0 else "draw"
                    ),
                    "pnl": result_val,
                }
            )
