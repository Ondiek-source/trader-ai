...
def test_atr14_ratio_change_finite_after_warmup(full_feature_df):
    col = full_feature_df["ATR_14_RATIO_CHANGE"]
    assert col.notna().all()
    assert np.isfinite(col.values).all()
...
def test_bb_width_ma20_non_negative(full_feature_df):
    assert (full_feature_df["BB_WIDTH_MA_20"] >= 0).all()
...
def test_derived_zero_filled_when_deps_missing(engineer):
    ...
    assert (result["ROC_5_ACCELERATION"] == 0.0).all()
    assert (result["ATR_14_RATIO_CHANGE"] == 0.0).all()
    assert (result["MACD_HIST_SLOPE"] == 0.0).all()
    assert (result["BB_WIDTH_MA_20"] == 0.0).all()
    assert (result["ATR_14_MA_20"] == 0.0).all()
