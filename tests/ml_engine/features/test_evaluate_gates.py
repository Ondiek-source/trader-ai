...
def _gate_df(**overrides):
    ...
    """
    defaults = {
        "BB_WIDTH": 0.020,       # > BB_WIDTH_MA_20 (0.010) → pass
        "BB_WIDTH_MA_20": 0.010,
        "ATR_14": 0.0010,        # > ATR_14_MA_20 (0.0005) → pass
        "ATR_14_MA_20": 0.0005,
...
def test_evaluate_gates_bb_fail_blocks_trade(engineer):
    fe = _gate_df(BB_WIDTH=0.005, BB_WIDTH_MA_20=0.020)  # BB_WIDTH < MA → fail
...
def test_evaluate_gates_atr_fail_blocks_trade(engineer):
    fe = _gate_df(ATR_14=0.0001, ATR_14_MA_20=0.001)  # ATR < MA → fail
    result = engineer.evaluate_gates(fe)
    assert result.is_eligible is False
