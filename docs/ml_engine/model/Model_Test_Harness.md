...
from Bar.**post_init** (lines 229-234 in the current file). Those lines
...
PHILOSOPHY:
model.py is a data firewall. Tests must verify two things equally: 1. Valid data passes through without modification (except tz stripping). 2. Invalid data is caught at the door -- no silent acceptance, no
...
## TEST 1.6 -- **repr** contains key fields

WHAT IT CHECKS:
The custom **repr** must include the symbol, bid, ask, and source
...
WHAT IT CHECKS:
If a timezone-aware datetime is passed in, **post_init** must
...
## TEST 4.6 -- **repr** contains key fields
...
WHAT IT CHECKS:
open == high is physically valid (a bar that opened at its
...
## TEST 6.10 -- **len** returns correct count
...
## TEST 7.4 -- **len** is accurate under concurrent modification

WHAT IT CHECKS:
len(buffer) must return a consistent value even while other
threads are adding ticks. Without the lock on **len**, it could
...
calls inside DataBuffer.**init** and add() -- covered by tests
6.2, 6.3, and 6.9 respectively.

KNOWN BLOCKER:
If the engine pseudocode block (Bar.**post_init** lines 229-234)
