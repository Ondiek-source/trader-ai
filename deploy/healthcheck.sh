...
elif echo "$LOGS" | grep -q "$_EVT_TICK"; then
  TICKS=$(echo "$LOGS" | grep "$_EVT_TICK" | tail -1 \
...
if echo "$LOGS" | grep "$_EVT_TICK" > /dev/null 2>&1; then
  TOTAL_TICKS=$(echo "$LOGS" | grep "$_EVT_TICK" | tail -1 \
    | grep -o "'ticks': [0-9]*" | head -1 | grep -o "[0-9]*")
  LATEST_PAIR=$(echo "$LOGS" | grep "$_EVT_TICK" | tail -1 \
